...
@IocBean
@At("/platform/hi/auth")
public class HitestController{
    private static final Log log = Logs.get();
    @Inject
    private HitestService gyAuthService;
    ...
    @SLog(tag = "hitest", msg = "${args[0].id}")
    public Object addDo(@Param("..")hitest gyAuth, HttpServletRequest req) {
		try {
    ...
    @SLog(tag = "hitest", msg = "${args[0].id}")
    public Object editDo(@Param("..")hitest gyAuth, HttpServletRequest req) {
		try {
    ...
    @SLog(tag = "hitest", msg = "${req.getAttribute('id')}")
