...
public class GyLoginController {
    ...
    public String login(HttpServletRequest req, HttpSession session) {
        ...
        if (subject.isAuthenticated()) {
            ...
            return "beetl:/public/login.html";

        }
    }

    @At("/noPermission")
    @Ok("beetl:/public/noPermission.html")
    @Filters
    ...
    public void theme(@Param("loginTheme") String theme, HttpServletRequest req) {
        if (!Strings.isEmpty(theme)) {
            ...
            if (subject != null) {
                ...
                userService.update(org.nutz.dao.Chain.make("loginTheme", theme), Cnd.where("id", "=", user.getId()));
            }
        }
    }
    ...
    public void layout(@Param("p") String p, @Param("v") boolean v, HttpServletRequest req) {
        ...
        if (subject != null) {
            Sys_user user = (Sys_user) subject.getPrincipal();
            if ("sidebar".equals(p)) {
                userService.update(org.nutz.dao.Chain.make("loginSidebar", v), Cnd.where("id", "=", user.getId()));
                user.setLoginSidebar(v);
            } else if ("boxed".equals(p)) {
                userService.update(org.nutz.dao.Chain.make("loginBoxed", v), Cnd.where("id", "=", user.getId()));
                user.setLoginBoxed(v);
            } else if ("scroll".equals(p)) {
                userService.update(org.nutz.dao.Chain.make("loginScroll", v), Cnd.where("id", "=", user.getId()));
    ...
    public Object doLogin(@Attr("loginToken") AuthenticationToken token, HttpServletRequest req, HttpSession session) {
        ...
        try {
            ...
            userService.update(org.nutz.dao.Chain.make("loginIp", user.getLoginIp()).add("loginAt", (int) (System.currentTimeMillis() / 1000))
    ...
    public void logout(HttpSession session) {
        try {
            ...
            if (user != null) {
                ...
                sysLog.setUsername(user.getUsername());
                sLogService.async(sysLog);
                userService.update(org.nutz.dao.Chain.make("isOnline", false), Cnd.where("id", "=", user.getId()));
            }
